...
use crate::{dexes::pumpfun::{parse_pumpfun_log, pumpfun_event_to_swap, PumpfunEventType, PUMPFUN_PROGRAM_ID}, transaction::{wrapper::TransactionWrapper, InstructionWrapper}};
...
impl  Parser for PumpfunParser {
  fn parse(&self, ix: &InstructionWrapper, tx: &TransactionWrapper, block: &BlockInfo) -> Result<ParserResult> {
    ...
    let logs = logs.iter().filter_map(|log| log.strip_prefix("Program data: ")).collect::<Vec<&str>>();
    ...
    match event {
      PumpfunEventType::Create(create_event ) => {
        ...
        return Ok(ParserResult{ parsed: true, ix_type: "Create".to_string(), data: ParserResultData::Token(create) });
      }
      PumpfunEventType::Trade(trade_event) => {
        let swap_info = pumpfun_event_to_swap(&trade_event, tx, *slot, *block_time)?;
        if swap_info.is_none() {
          return Ok(ParserResult{ parsed: false, ix_type: "Trade".to_string(), data: ParserResultData::NoData });
        }
        let swap_info = swap_info.unwrap();
        let ix_type = format!("Trade{}", swap_info.swap_type.to_db());
        return Ok(ParserResult{ parsed: true, ix_type, data: ParserResultData::Swap(swap_info) });
      }
      PumpfunEventType::SetParams(_) => {
        return Ok(ParserResult{ parsed: true, ix_type: "SetParams".to_string(), data: ParserResultData::NoData });
      }
      PumpfunEventType::Complete(_) => {
        return Ok(ParserResult{ parsed: true, ix_type: "Complete".to_string(), data: ParserResultData::NoData });
      }
    }
  }
}
