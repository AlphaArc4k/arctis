use crate::transaction::{parse_ui_instruction, wrapper::TransactionWrapper, InstructionWrapper};
use anyhow::{Ok, Result};
use arctis_types::{AccountInfo, BlockInfo, ParserResult, ParserResultData, SplTokenTransfer, SupplyChange};
...
impl Parser for TokenProgramParser {
  fn parse(&self, ix: &InstructionWrapper, tx: &TransactionWrapper, block: &BlockInfo) -> Result<ParserResult> {
    ...
    match ix_type {
        ...
        "approve" => {
          return Ok(ParserResult{ parsed: false, ix_type: "approve".to_string(), data: ParserResultData::NoData });
        }
        ...
        "closeAccount" => {
          ...
          if ix_parsed.parsed["info"]["owner"].as_str().is_none() {
            return Err(anyhow::anyhow!("closeAccount: multisig account not supported"));
          }

          let mut account_info = AccountInfo {
            account: ix_parsed.parsed["info"]["account"].as_str().unwrap().to_string(),
            owner: ix_parsed.parsed["info"]["owner"].as_str().unwrap().to_string(),
            open_tx: None,
            init_tx: None,
            close_tx: Some(signature),
            close_destination: Some(ix_parsed.parsed["info"]["destination"].as_str().unwrap().to_string()),
            mint: None,
            decimals: None,
          };
        ...
        "initializeAccount" => {
          ...
          let mut account_info = AccountInfo {
            ...
            mint: Some(ix_parsed.parsed["info"]["mint"].as_str().unwrap().to_string()),
            decimals: None,
          };
        ...
        "initializeAccount2" => {
          return Ok(ParserResult{ parsed: false, ix_type: "initializeAccount2".to_string(), data: ParserResultData::NoData });
        }
        "initializeAccount3" => {
          return Ok(ParserResult{ parsed: false, ix_type: "initializeAccount3".to_string(), data: ParserResultData::NoData });
        }
        "initializeImmutableOwner" => {
          return Ok(ParserResult{ parsed: false, ix_type: "initializeImmutableOwner".to_string(), data: ParserResultData::NoData });
        }
        "approveChecked" => {
          return Ok(ParserResult{ parsed: false, ix_type: "approveChecked".to_string(), data: ParserResultData::NoData });
        }
        ...
        "syncNative" => {
          ...
          return Ok(ParserResult{ parsed: true, ix_type: "syncNative".to_string(), data: ParserResultData::NoData });
        }
        "initializeMint" => {
          return Ok(ParserResult{ parsed: false, ix_type: "initializeMint".to_string(), data: ParserResultData::NoData });
        }
        "mintTo" => {
          return Ok(ParserResult{ parsed: false, ix_type: "mintTo".to_string(), data: ParserResultData::NoData });
        }
        "mintToChecked" => {
          return Ok(ParserResult{ parsed: false, ix_type: "mintToChecked".to_string(), data: ParserResultData::NoData });
        }
        "burn" => {
        ...
        "burnChecked" => {
          return Ok(ParserResult{ parsed: false, ix_type: "burnChecked".to_string(), data: ParserResultData::NoData });
        }
        "setAuthority" => {
          return Ok(ParserResult{ parsed: false, ix_type: "setAuthority".to_string(), data: ParserResultData::NoData });
        }
        "revoke" => {
          return Ok(ParserResult{ parsed: false, ix_type: "revoke".to_string(), data: ParserResultData::NoData });
        }
        _ => {
          return Ok(ParserResult{ parsed: false, ix_type: ix_type.to_string(), data: ParserResultData::NoData });
        }
    }
  }
}

fn parse_transfer(parsed: &serde_json::Value, tx: &TransactionWrapper, block_info: &BlockInfo, signature: String) -> SplTokenTransfer {
  ...
  let amount = match parsed["amount"].as_str() {
    ...
    // transfer_checked:
    None => parsed["tokenAmount"]["amount"].as_str().unwrap().parse::<f64>().unwrap(),
  };
