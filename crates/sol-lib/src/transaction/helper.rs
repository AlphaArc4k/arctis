...
pub fn get_transaction_message(transaction: &EncodedTransactionWithStatusMeta) -> &UiRawMessage {
    ...
    match &tx.message {
        solana_transaction_status::UiMessage::Raw(message) => message,
...
pub fn get_token_decimals(
    ...
    let pre_token_balances = transaction
        .meta
        .as_ref()
        .unwrap()
        .pre_token_balances
...
fn get_transaction_instructions_with_inner(
    transaction: &EncodedTransactionWithStatusMeta,
    ...
    for instruction in &message.instructions {
        ...
        let inner_instruction = inner_instructions
            ...
            .unwrap_or(&def)
            .iter()
            .find(|inner_instruction| inner_instruction.index == i);
        let inner_instructions =
            inner_instruction.map_or(vec![], |inner| inner.instructions.clone());
        ...
        for inner_instruction in inner_instructions {
            let inner_compiled_instruction = match inner_instruction {
                ...
                UiInstruction::Compiled(instruction) => instruction.clone(),
            };
