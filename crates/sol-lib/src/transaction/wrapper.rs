...
use solana_transaction_status::{option_serializer::OptionSerializer, EncodedTransactionWithStatusMeta, UiCompiledInstruction, UiRawMessage, UiTransaction, UiTransactionStatusMeta};
use anyhow::Result;

use super::helper::{get_accounts, get_inner_instructions, get_token_account_lookup, get_token_decimals, get_transaction_data, get_transaction_message, get_transaction_meta, get_transaction_signature, get_transaction_signatures, has_error, TokenAccountInfo};
...
impl TransactionWrapper {
  ...
  pub fn get_compute_units_consumed(&self) -> u64 {
    return self.tx.meta.as_ref().unwrap().compute_units_consumed.clone().unwrap_or(0);
  }
  ...
  pub fn get_inner_ix_count(&self) -> u8 {
    ...
    return meta.inner_instructions.as_ref().map_or(0, |inner| inner.len() as u8);
  }
