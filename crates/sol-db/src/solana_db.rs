...
fn create_connection(file_path: Option<&str>, use_primary_keys: bool) -> Result<Connection> {
  ...
  conn.execute_batch(
      ...
      if use_primary_keys { ", PRIMARY KEY (signature, ix_index)" } else { "" }, // supply_changes
      if use_primary_keys { "PRIMARY KEY" } else { "" }, // fees
      if use_primary_keys { "PRIMARY KEY" } else { "" }, // cant_discard
      if use_primary_keys { ", PRIMARY KEY (signature, ix_index)" } else { "" }, // tx_programs
      ).as_str(),
  )?;
...
impl SolanaDatabase {
  pub fn new() -> Result<SolanaDatabase> {
    ...
    Ok(SolanaDatabase { conn, no_op: false, path: None, use_primary_keys: true })
  }
  ...
  pub fn new_with_config(config: DatabaseConfig) -> Result<SolanaDatabase> {
    let conn = match config.mode {
      ...
      DatabaseMode::File => create_connection(config.path.as_deref(), config.with_primary_keys)?,
    };
    let mut db = SolanaDatabase { conn, no_op: false, path: config.path, use_primary_keys: config.with_primary_keys };
    if config.enable_s3 {
      db.enable_s3();
    }
    Ok(db)
  ...
  pub fn new_with_primary_keys(with_primary_keys: bool) -> Result<SolanaDatabase> {
    ...
    Ok(SolanaDatabase { conn, no_op: false, path: None, use_primary_keys: with_primary_keys })
  }

  pub fn new_from_file(file_path: &str) -> Result<SolanaDatabase> {
    let conn = create_connection(Some(file_path), true)?;
    Ok(SolanaDatabase { conn, no_op: false, path: Some(file_path.to_string()), use_primary_keys: true })
  }

  pub fn new_from_file_with_primary_keys(file_path: &str, with_primary_keys: bool) -> Result<SolanaDatabase> {
    let conn = create_connection(Some(file_path), with_primary_keys)?;
    Ok(SolanaDatabase { conn, no_op: false, path: Some(file_path.to_string()), use_primary_keys: with_primary_keys })
  }
  ...
  pub fn new_from_connection(conn: Connection) -> SolanaDatabase {
    ...
    SolanaDatabase { conn, no_op: false, path: None, use_primary_keys: true }
  }
        ...
        match path.file_name() {
          Some(file_name) => {
            if with_extension {
              ...
              Some(file_name.to_str().unwrap().replace(&format!(".{}", extension.to_str().unwrap()), ""))
            }
          },
          None => None,
        }
  ...
  pub fn query_to_json_file(&self, query: &str) -> Result<Vec<Value>> {
    let ts_now = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis();
  ...
  pub fn insert_transactions_bulk(&mut self, transactions: &Vec<ProcessedTransaction>) -> Result<usize> {
    let conn = &self.conn;
    ...
    for transaction in transactions {
      appender.append_row(params![
        ...
        match &transaction.data {
          Some(data) => Some(serde_json::to_string(data).unwrap()),
          None => None
        }
      ])?;
    }
  ...
  pub fn insert_token_transfers_bulk(&mut self, transfers: &Vec<&SplTokenTransfer>) -> Result<usize> {
    let conn = &self.conn;
  ...
  pub fn insert_supply_changes_bulk(&mut self, supply_changes: &Vec<&SupplyChange>) -> Result<usize> {
    let conn = &self.conn;
  ...
  pub fn insert_parsed_programs_bulk(&mut self, programs: &Vec<&ProgramParserData>) -> Result<usize> {
    let conn = &self.conn;
  ...
  pub fn insert_compute_budget_bulk(&mut self, budget: &Vec<ComputeBudgetProcessed>) -> Result<usize> {
    let conn = &self.conn;
  ...
  pub fn load_parquet_table(&self, table: &str, file_path: &str) -> Result<()> {
    ...
    let _ = connection.execute(
      format!("COPY {} FROM '{}' (FORMAT 'parquet', COMPRESSION 'ZSTD');", table, file_path).as_str(), 
      []
    )?; 
    Ok(())
  }
