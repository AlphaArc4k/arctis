...
use sol_lib::{self as sol, transaction::{wrapper::TransactionWrapper, InstructionWrapper}};
...
pub fn process_transaction(
  ...
  */
  ...
  for ix_idx in 0..ix_len {
    ...
    let program_ix_index = program_indexes.entry(program_id.clone()).and_modify(|e| { *e += 1 }).or_insert(0);

    // get parser for program based on id
    let parser = sol::transaction::parsers::get_parser(&program_id);
    if parser.is_none() {
      parsed_programs.push(ProgramParserData { signature: signature.clone(), ix_idx, program_id: program_id.clone(), ix_type: "no_parser".to_string(), parsed: false, error: false, });
      can_discard = false;
      continue;
    }
    ...
    if result.is_err() {
      ...
      parsed_programs.push(ProgramParserData { signature: signature.clone(), ix_idx, program_id: program_id.clone(), ix_type: "unknown".to_string(), parsed: false, error: true, });
      can_discard = false;
      continue;
    }
    let result = result.unwrap();
    let parsed = result.parsed;
    parsed_programs.push(ProgramParserData { signature: signature.clone(), ix_idx, program_id: program_id.clone(), ix_type: result.ix_type.clone(), parsed, error: false, });
    
    let ix_type = &result.ix_type;
    let _can_discard = match &result.data {
      ParserResultData::NoData => {
        match ix_type.as_str() {
          "syncNative" => {
            true
          },
          "sequence_enforcer" => {
